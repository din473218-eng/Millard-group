          ...
          {brandCards.map((brand, idx) => {
            ...
            return (
              <div
                ...
                className="group relative flex flex-col bg-brand-beige border border-brand-gold/15 hover:border-brand-gold/45 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-2 h-full rounded-none"
              >
                {/* Editorial Header bar inside card matching Design HTML */}
                <div className="px-6 pt-6 pb-2 flex justify-between items-center bg-brand-beige">
                ...
                {/* Image Section */}
                <div className="relative aspect-[4/3] w-full overflow-hidden px-6 bg-brand-beige">
                  <div className="w-full h-full overflow-hidden border border-brand-gold/10 relative">
                    <img
                      src={brand.image}
                      alt={brand.name}
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-brand-beige/20 via-transparent to-transparent" />
                  </div>
                  
                  {/* Floating Icon Badges */}
                  <div className="absolute top-6 right-10 p-2 rounded-none bg-white/90 backdrop-blur-md border border-brand-gold/20 shadow-sm z-10">
                ...
                <div className="p-6 md:p-8 flex flex-col flex-grow justify-between space-y-6">
                    ...
                    <div className="space-y-2 pt-3 border-t border-brand-gold/10">
                      {brand.highlights.map((highlight, index) => (
                        <div key={index} className="flex items-center space-x-2 text-brand-brown">
                          <span className="w-1.5 h-1.5 rounded-none bg-brand-gold/75 rotate-45" />
                  ...
                  <div className="pt-4">
                    <button
                      ...
                      className="w-full py-3 bg-brand-charcoal hover:bg-brand-gold text-white font-mono text-[10px] tracking-widest uppercase font-semibold transition-all duration-300 flex items-center justify-center space-x-2 cursor-pointer focus:outline-none rounded-none border border-transparent shadow-sm"
                    >
